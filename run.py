...
if __name__ == '__main__':
    ...
    if show:
            show_match(my_bot, opponents[2], 16)
    else:
            # use this command if you just want the results of the matches reported
            counter = test(my_bot, opponents[2], 16)
            if(counter == 1):
                winCounter['player1'] = winCounter['player1'] + 1
            else:
                winCounter['player2'] = winCounter['player2'] + 1
            
            #test(my_bot, opponents[1], map)
    print(winCounter)        
